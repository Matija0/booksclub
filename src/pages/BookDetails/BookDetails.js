...
const BookDetails = () => {
  ...
  const addBook = (data) => {
    if (Object.keys(library).length < 1) {
      setLibrary(data)
    }
    else {
      setLibrary((vals) => {
  ...
  if (loading) {
    return (<div className=" bg-white container mx-auto mt-12 flex flex-row justify-center"><img className=" w-1/4" src={Loader} /></div>)
  }
  if (!bookData) {
  ...
  return (
    <>
      ...
      <div className=" container mx-auto bg-gray-200 mt-5 p-5 rounded-lg flex flex-row gap-5 mb-7 ">
        ...
        <div className=" bg-slate-200 flex flex-col items-start w-3/5">
          ...
          <button className=" border-none rounded-lg bg-orange-400 text-white py-2 px-3 mt-4 hover:bg-orange-300" onClick={() => addBook(bookData)}>
            Add to Library
          </button>
        </div>
      </div>
